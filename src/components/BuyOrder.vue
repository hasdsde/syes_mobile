                ...
                >
                  ...
                  <template v-slot:navigation>
                    <q-stepper-navigation v-if="item.status!==3">
                      <q-btn @click="checkStatus(item.id)" color="primary" v-if="item.status===1"
                             label="确认交易"/>
...
//加载页面
function loadPage() {
  api.get('/about/BuyOrder?pagesize=' + pageSize.value + '&currentpage=' + currentPage.value).then(res => {
    ...
    actions: [
      {
        label: '确定', color: 'yellow', handler: () => {
          api.get('/order/status?id=' + value1).then(res => {
            $router.go(0)
          })
        }
      },
